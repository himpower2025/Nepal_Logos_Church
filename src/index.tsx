...
const MCCHEYNE_READING_PLAN = [
    ...
    "लेवी ८, यूहन्ना ५:२४-४७, भजनसंग्रह ३０, १ थिस्सलोनिकी ५",
    ...
    "२ शमूएल ४, प्रेरित १७, यशैया ५３, भजनसंग्रह ३२",
    ...
    "२ शमूएल १०, प्रेरित २३, यशैया ५९, भजनसंग्रह ३８",
        ...
        <div className="page-content">
            ...
            <div className="past-worship-section">
                ...
                {pastServices.length > 0 ? (
                    ...
                    <p>विगतको आरधना भिडियोहरू छैनन्।</p>
                )}
            </div>
    ...
    return (
        <Modal onClose={onClose} position="bottom">
            <form className="modal-form" onSubmit={handleSubmit}>
                <h3>विगतको आरधना थप्नुहोस्</h3>
                <input type="text" placeholder="आरधना शीर्षक" value={title} onChange={e => setTitle(e.target.value)} required />
                <input type="url" placeholder="आरधना Link" value={youtubeUrl} onChange={e => setYoutubeUrl(e.target.value)} required />
                <button type="submit" className="action-button" disabled={isSubmitting}>
                    {isSubmitting ? "थप्दै..." : "थप्नुहोस्"}
                </button>
            </form>
        </Modal>
    );
    ...
    const handleStartRecording = async () => {
        if (navigator.mediaDevices && navigator.mediaDevices.getUserMedia) {
            try {
                ...
                alert("माइक पहुँच गर्न सकिएन। कृपया यो साइटको लागि आफ्नो ब्राउजर अनुमतिहरू जाँच गर्नुहोस्।");
            }
        } else {
             alert("तपाईंको ब्राउजरले अडियो रेकर्डिङ समर्थन गर्दैन।");
        }
    };
        ...
        <Modal onClose={onClose} position="bottom">
            <form className="modal-form" onSubmit={handleSubmit}>
                ...
                <div className="add-podcast-tabs">
                    <button type="button" className={activeTab === 'upload' ? 'active' : ''} onClick={() => setActiveTab('upload')}>Upload the File</button>
                    <button type="button" className={activeTab === 'record' ? 'active' : ''} onClick={() => setActiveTab('record')}>Live Recording</button>
                </div>
                    ...
                    <div className="record-section">
                        {!isRecording && !recordedAudioUrl && (
                            <button type="button" className="record-button" onClick={handleStartRecording}>
                                <span className="material-symbols-outlined">mic</span> Record
                            </button>
                        )}
                        {isRecording && (
                        ...
                        {recordedAudioUrl && !isRecording && (
                            <div className="recording-preview">
                                ...
                                <button type="button" className="action-button secondary" onClick={handleStartRecording}>Record Again</button>
                            </div>
                        )}
                    </div>
                )}
...
const App = () => {
    ...
    const handleSavePrayerRequest = async (data: { title: string; content: string; imageFile: File | null; imageRemoved: boolean }, id?: string) => {
        if (!currentUser || !db || !storage) return Promise.reject("Services not ready");
    
        const { title, content, imageFile, imageRemoved } = data;
        const isEditing = !!id;
        
        let finalImageUrl: string | null = isEditing ? prayerRequests.find(p=>p.id === id)?.image ?? null : null;

        try {
            if (imageFile) {
                if (isEditing) {
                    const existingImageUrl = prayerRequests.find(p => p.id === id)?.image;
                    if (existingImageUrl) {
            ...
            } else if (imageRemoved && isEditing) {
                const existingImageUrl = prayerRequests.find(p => p.id === id)?.image;
                if (existingImageUrl) {
            ...
            console.error("Error uploading prayer image:", error);
            alert("फोटो अपलोड गर्न असफल भयो। कृपया फेरि प्रयास गर्नुहोस्।");
            throw error;
        }
        ...
        try {
            ...
            console.error("Error saving prayer request to DB:", error);
            alert("डाटाबेसमा प्रार्थना अनुरोध बचत गर्न असफल भयो।");
            throw error;
        }
    };
    ...
    const handleAddNews = async (data: {title: string, content: string, imageFile: File | null}) => {
        ...
        try {
            ...
            alert("डाटाबेसमा समाचार बचत गर्न असफल भयो।");
            throw error;
        }
    };
    ...
    const handleAddPodcast = async (data: {title: string, audioFile: File}) => {
        ...
        try {
            ...
            audioUrl = await getDownloadURL(uploadResult.ref);
             ...
             alert("अडियो फाइल अपलोड गर्न असफल भयो। कृपया फेरि प्रयास गर्नुहोस्।");
             throw error;
        }
        ...
        try {
            ...
            console.error("Error saving podcast to DB: ", error);
            alert("डाटाबेसमा पोडकास्ट बचत गर्न असफल भयो।");
            throw error;
        }
    };
    ...
    const handleStartChat = async (otherUserIds: string[]) => {
        ...
        try {
            ...
            alert("कुराकानी सुरु गर्न असफल भयो।");
        } finally {
