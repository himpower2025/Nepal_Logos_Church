            ...
            <div className="message-list">
                {chat.messages.map(msg => (
                    <div key={msg.id} className={`message-container ${msg.sender.id === CURRENT_USER.id ? 'sent' : 'received'}`}>
                        {chat.isGroup && msg.sender.id !== CURRENT_USER.id && (
...
const CreateChatModal = ({ onClose, onCreateChat }: { onClose: () => void; onCreateChat: (data: { userIds: string[], name?: string }) => void; }) => {
    ...
    const handleCreate = () => {
        ...
        onCreateChat({ userIds: selectedUserIds, name: groupName.trim() });
    };
        ...
        <Modal onClose={onClose}>
            <div className="create-chat-modal">
                ...
                <div className="new-chat-options">
                 ...
                 <button className="action-button create-chat-button" onClick={handleCreate} disabled={selectedUserIds.length === 0}>
                    Start Chat
                </button>
            </div>
        </Modal>
...
const App = () => {
    ...
    const handleCreateChat = ({ userIds, name }: { userIds: string[], name?: string }) => {
        const allParticipantIds = [CURRENT_USER.id, ...userIds];
        ...
        if (uniqueParticipantIds.length === 2) {
            const existingChat = chats.find(chat => 
                ...
                chat.participants.every(p => uniqueParticipantIds.includes(p.id))
            );
            if (existingChat) {
        ...
        const participants = MOCK_USERS.filter(u => uniqueParticipantIds.includes(u.id));
        const isGroup = participants.length > 2;
        const groupName = isGroup ? (name || participants.filter(p=>p.id !== CURRENT_USER.id).map(p => p.name).join(', ')) : undefined;
