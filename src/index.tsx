        ...
        <div className="page-content chat-list-page">
            ...
            <div className="list-container">
                {chats.map(chat => {
                    ...
                    let isUnread = false;
                    // Safely check if both are valid Timestamp objects before comparing
                    if (lastMessageCreatedAt instanceof Timestamp) {
                        if (lastReadTimestamp instanceof Timestamp) {
                            isUnread = lastReadTimestamp.toMillis() < lastMessageCreatedAt.toMillis();
                        } else if (!lastReadTimestamp) {
                            // If there's a last message but the user has never read the chat, it's unread.
                            isUnread = true;
                        }
                    }
...
const App = () => {
    ...
    const handleSavePrayerRequest = async (data: { title: string; content: string; imageFile: File | null; imageRemoved: boolean }, id?: string) => {
        ...
        try {
            const isEditing = !!id;
            const originalRequest = isEditing ? prayerRequests.find(p => p.id === id) : undefined;
            let finalImageUrl: string | null = originalRequest?.image || null;
    
            // Step 1: Handle new image upload if present
            if (data.imageFile) {
                const newImageRef = ref(storage, `prayer_images/${Date.now()}_${data.imageFile.name}`);
            ...
            // Step 2: Delete old image if a new one was uploaded or it was removed
            const oldImageUrl = originalRequest?.image;
            if (oldImageUrl && oldImageUrl !== finalImageUrl) {
                await deleteObject(ref(storage, oldImageUrl)).catch(e => console.warn("Old image deletion failed, but proceeding.", e));
            }
            
            // Step 3: Prepare Firestore data
            const payload: Omit<PrayerRequest, 'id' | 'authorId' | 'authorName' | 'prayedBy' | 'comments' | 'createdAt'> = {
                title: data.title,
                content: data.content,
                image: finalImageUrl,
            };
    
            // Step 4: Update or Add document
            if (isEditing && id) {
                ...
                await addDoc(collection(db, "prayerRequests"), fullPayload);
            }
        } catch (error) {
